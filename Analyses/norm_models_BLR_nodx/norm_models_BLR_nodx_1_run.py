...
# INCLUSIONS: only participants without any dx EXCEPT depressive episode, other anxiety disorder, migraine, and haemmoroids (high prevalence, low impact on brain)
ids = dt.fread("../../QC/inclusions_nodx_inputNM.txt").to_pandas()
...
B = create_bspline_basis(demo['Age'].min() - 1, demo['Age'].max() + 1, p=3, nknots = 4)
...
# fig = (pn.ggplot() +
# pn.geom_point(vox, pn.aes(x="Age", y="column_146197"), color="blue", alpha=0.01) + 
# pn.geom_line(demo_pred, pn.aes(x="Age", y="Mean", color="factor(Sex)")) +
# pn.geom_line(demo_pred, pn.aes(x="Age", y=(demo_pred['Mean'] + (demo_pred['SD'])), color="factor(Sex)"), alpha=0.8) +
# pn.geom_line(demo_pred, pn.aes(x="Age", y=(demo_pred['Mean'] - (demo_pred['SD'])), color="factor(Sex)"), alpha=0.8)
# # pn.geom_smooth(vox, pn.aes(x="Age", y="column_146197", color="factor(Sex)"))
# )
# fig.save("test_p4_nk5.png", dpi=300)
